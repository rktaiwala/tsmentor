...
class Lazy_Query_Manager {
	 ...
	 * Ajax action name
	 */
	const ACTION = 'ts_get_lazy_query_data';

	/**
	 * Taxonomy tems query
	 */
	const QUERY_TERMS = 'terms';
	const QUERY_CHILD_TERMS = 'child_terms';
	const QUERY_TAX = 'taxonomies';
	...
	protected static function get_query_handler() {
		$query = isset( $_POST['query'] ) ? $_POST['query'] : self::QUERY_POSTS;
		$handlers = [
			self::QUERY_POSTS   => 'get_posts',
			self::QUERY_TERMS   => 'get_terms',
			self::QUERY_CHILD_TERMS   => 'get_child_terms',
			self::QUERY_TAX   => 'get_tax',
    ...
    public static function get_tax() {
        ...
        $output = 'objects';
        error_log(self::get_post_type());
		if ( self::get_post_type() ) {
			//$taxonomies = get_taxonomies( array('object_type' => array( self::get_post_type())),$output );
            $taxonomies = get_object_taxonomies(  self::get_post_type(),$output );
		}
