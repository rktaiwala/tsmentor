(function ($) {
    $(document).on('ts_select2_init', function (event, obj) {
        var ID = '#elementor-control-default-' + obj.data._cid;
        setTimeout(function () {
            var IDSelect2 = $(ID).select2({
                minimumInputLength: 3,
                ajax: {
                    url: ts_select2_localize.ajaxurl + "?action=ts_select2_search_post&post_type=" + obj.data.source_type + '&source_name=' + obj.data.source_name,
                    dataType: 'json'
                },
                initSelection: function (element, callback) {
                    if (!obj.multiple) {
                        callback({id: '', text: ts_select2_localize.search_text});
                    }else{
						callback({id: '', text: ''});
					}
					var ids = [];
                    ...
                    if (ids.length > 0) {
                        ...
                        $.ajax({
                            method: "POST",
                            url: ts_select2_localize.ajaxurl + "?action=ts_select2_get_title",
                            ...
                            if (response.success && typeof response.data.results != 'undefined') {
                                ...
                                element.append(tsSelect2Options);
                            }
